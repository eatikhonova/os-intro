...
![Необходимо установить редактор Emacs](image/1.png){ #fig:001 width=70% }
...
![Создадим файл lab10.sh и наберем необходимый текст](image/3.png){ #fig:001 width=70% }
...
![Вырежем командой целую строку и вставим строку в конец файла](image/4.png){ #fig:001 width=70% }

3.	Выделим область текста («Ctrl-space»).
4.	Скопируем область в буфер обмена («Alt-w»).
5.	Вставим область в конец файла («Ctrl-y») 

![Скопируем область в буфер обмена и вставим область в конец файла](image/5.png){ #fig:001 width=70% }

6.	Вновь выделим эту область («Ctrl-space») и на этот раз вырежем её («Ctrl-w») 

![Вновь выделим эту область и вырежем её](image/6.png){ #fig:001 width=70% }

7.	Отменим последнее действие («Ctrl-/») 

![Отменим последнее действие](image/7.png){ #fig:001 width=70% }

	1.Переместим курсор в начало строки («Ctrl-a»)

![Переместим курсор в начало строки](image/8.png){ #fig:001 width=70% }

	2.Переместим курсор в конец строки («Ctrl-e») 

![Переместим курсор в конец строки](image/9.png){ #fig:001 width=70% }

    3.Переместим курсор в начало буфера («Alt-<») 

![Переместим курсор в начало буфера](image/10.png){ #fig:001 width=70% }

    4.Переместим курсор в конец буфера («Alt->») 

![Переместим курсор в конец буфера](image/11.png){ #fig:001 width=70% }

    1.Выведем список активных буферов на экран («Ctrl-x» «Ctrl-b») 

![Выведем список активных буферов на экран](image/12.png){ #fig:001 width=70% }
...
![Переместимся в окно со списком открытых буферов](image/13.png){ #fig:001 width=70% }

![Переключимся на другой буфер](image/14.png){ #fig:001 width=70% }

    3.Закроем это окно («Ctrl-x 0»).
    4.Теперь вновь переключимся между буферами, но уже без вывода их списка на экран («Ctrl-x b»)

![Закроем окно](image/15.png){ #fig:001 width=70% }
![Переключимся между буферами без вывода списка на экран](image/16.png){ #fig:001 width=70% }
...
![В каждом из четырёх созданных окон откроем новый буфер](image/17.png){ #fig:001 width=70% }

10.	1.Переключимся в режим поиска («Ctrl-s») и найдем несколько слов, присутствующих в тексте 

![Переключимся в режим поиска и найдем несколько слов](image/18.png){ #fig:001 width=70% }

    2.Переключимся между результатами поиска, нажимая «Ctrl-s» 

![Переключимся между результатами поиска](image/19.png){ #fig:001 width=70% }
...
![Введем текст для замены](image/20.png){ #fig:001 width=70% }

![Нажмем «!» для подтверждения замены](image/21.png){ #fig:001 width=70% }

    5.Пробуем другой режим поиска, нажав «Alt-s o» 

![Пробуем другой режим поиска](image/22.png){ #fig:001 width=70% }
